document.addEventListener('DOMContentLoaded', () => {
    ...
    const saveButton = document.querySelector('#btnSave'); // Missing saveButton reference
    const userList = document.querySelector('.user-list'); // Move declaration here
    ...
    deleteButton.addEventListener('click', (event) => {
        ...
        // Find the user id from the form
        const userId = document.querySelector('#userID').value;
        const userIndex = userData.findIndex(user => user.id == userId);

        // Remove the user from the array and re-render the list
        if (userIndex !== -1) {
    ...
    saveButton.addEventListener('click', (event) => {
        ...
        for (let i = 0; i < userData.length; i++) {
            // Found the relevant user, update object at this index and redisplay
            if (userData[i].id == id) {
    ...
    function generateUserList(users, stocks) {
        // Get the list element and clear it first
    ...
    // Attach the click handler to the user list
